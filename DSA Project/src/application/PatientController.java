...
public class PatientController {
    ...
    private static final Map<String, PriorityQueue<Appointment>> appointmentCache = new HashMap<>();
    ...
    private void handleViewProfile(ActionEvent event) {
        ...
        if (!patientName.isEmpty()) {
            try {
                ...
                PriorityQueue<Appointment> appointmentsQueue = appointmentCache.get(patientName);
                ...
                if (appointmentsQueue == null) {
                    ...
                    // Use PriorityQueue to store appointments sorted by appointment time
                    appointmentsQueue = new PriorityQueue<>(Comparator.comparing(Appointment::getTime));

                    // Add all appointments to the queue
                    for (Appointment appointment : appointments) {
                        appointmentsQueue.add(appointment);
                    }

                    // Store the sorted queue in the cache for future access
                    appointmentCache.put(patientName, appointmentsQueue);
                }

                if (appointmentsQueue != null && !appointmentsQueue.isEmpty()) {
                    StringBuilder details = new StringBuilder();
                    while (!appointmentsQueue.isEmpty()) {
                        Appointment appointment = appointmentsQueue.poll();  // Retrieve the next appointment in sorted order
                        ...
                        details.append("Ward: ").append(appointment.getWard()).append("\n\n");
                    }
